...
// Copyright (c) 2015 Babacoin developers
// Copyright (c) 2015 Babacoin developers
...
/** Amount of babacoins spent by the transaction.
...
static const int64 nStartSubsidy = 20000 * COIN;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BabacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BabacoinMiner : ProcessBlock, block not accepted");
    }
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running BabacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BabacoinMiner terminated\n");
        throw;
    }
}
